...
executables = ["raycast_ssr_cuda_bvh", "raycast_ssr_cuda_bvh_binned"]
...
def profile(executable, input_file="gen.txt"):
    ...
    print(("{} Execution time: {} seconds".format(executable, time)))
...
if __name__ == "__main__":
    ...
    # 1> argument, profile specified input files
        ...
        for filename in sys.argv[1:]:
            ...
            row = []
            for executable in executables:
                metrics, time = profile("./"+executable, filename)
                row.append(float(metrics["flop_count_sp"]) / time)
                for metric in metric_names:
                    row.append(metrics[metric])
                sys.stderr.write(".")
