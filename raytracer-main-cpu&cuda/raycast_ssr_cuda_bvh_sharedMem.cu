// raycast_ssr_cuda_bvh_tiled_shmem.cu
// Tile-based version with SHARED MEMORY optimizations
// 
// Key optimizations:
// 1. Cooperative loading of scene data into shared memory
// 2. BVH node caching for coherent traversal
...
// Compact ray for register efficiency
struct CompactRay {
    float ox, oy, oz;
    float dx, dy, dz;
...
// ---------------------------
// SHARED MEMORY BVH TRAVERSAL
// Uses cached nodes for improved performance
// ---------------------------
__device__ bool intersectBVH_shared(const CompactRay &ray,
                                    const DeviceBVHNode* __restrict__ nodes,
                                    int numNodes,
                                    const int* __restrict__ triIndices,
                                    const DeviceTriangle* __restrict__ triangles,
                                    DeviceBVHNode* __restrict__ s_nodeCache,
                                    int cacheSize,
                                    float &closestT,
                                    int &hitTriIndex,
                                    Vec3d &hitNormal,
    ...
    while (stackPtr > 0) {
        ...
        // Try to fetch from shared memory cache first
        const DeviceBVHNode *node;
        if (nodeIdx < cacheSize) {
            node = &s_nodeCache[nodeIdx];
        } else {
            node = &nodes[nodeIdx];
        }
        ...
        if (node->isLeaf) {
            ...
            if (leftFirst) {
                if (node->right >= 0 && stackPtr < 31) stack[stackPtr++] = node->right;
                if (node->left >= 0 && stackPtr < 31) stack[stackPtr++] = node->left;
            } else {
                if (node->left >= 0 && stackPtr < 31) stack[stackPtr++] = node->left;
                if (node->right >= 0 && stackPtr < 31) stack[stackPtr++] = node->right;
            }
...
// Shadow ray version with shared memory
__device__ bool intersectBVH_shadow_shared(const CompactRay &ray,
                                           const DeviceBVHNode* __restrict__ nodes,
                                           int numNodes,
                                           const int* __restrict__ triIndices,
                                           const DeviceTriangle* __restrict__ triangles,
...
__device__ float computeShadowFactor_shared(const Vec3d &point, const Vec3d &lightDir, float maxDist,
                                            const DeviceSphere* s_spheres, int numSpheres,
                                            const DeviceBVHNode* bvhNodes, int numBVHNodes,
                                            const int* bvhTriIndices, const DeviceTriangle* triangles,
                                            DeviceBVHNode* s_nodeCache, int cacheSize) {
    Vec3d shadowOrig = point + lightDir * 1e-4f;
    CompactRay shadowRay = makeRay(shadowOrig, lightDir);
    float transparency = 1.0f;

    // Test against spheres in shared memory
    for (int i = 0; i < numSpheres; ++i) {
        float t;
        Vec2d texCoord;
        if (intersectRaySphere_device(shadowRay, s_spheres[i], t, texCoord) &&
            t > 0.001f && t < maxDist) {
            transparency *= (1.0f - s_spheres[i].material.alpha);
            if (transparency <= 0.01f) return 0.0f;
        }
    }

    if (bvhNodes && numBVHNodes > 0) {
        if (intersectBVH_shadow_shared(shadowRay, bvhNodes, numBVHNodes, bvhTriIndices, 
                                       triangles, s_nodeCache, cacheSize, maxDist)) {
            return 0.0f;
        }
    }
    return transparency;
...
__device__ Color getSSRColor(const Vec3d &reflectDir, const Vec3d &hitPoint,
                             const Color* imageBuffer, const float* depthBuffer,
    ...
    Vec3d currentPos = hitPoint + reflectDir * 0.01f;
...
__device__ Color shade_shared(const Vec3d &point, const Vec3d &rayDir, const Vec3d &normal,
                              const Vec2d &texCoord, const MaterialColor &material,
                              const DeviceLight* s_lights, int numLights,
                              const DeviceSphere* s_spheres, int numSpheres,
                              const DeviceBVHNode* bvhNodes, int numBVHNodes,
                              const int* bvhTriIndices, const DeviceTriangle* triangles,
                              DeviceBVHNode* s_nodeCache, int cacheSize,
    ...
    // Use lights from shared memory
    for (int i = 0; i < numLights; ++i) {
        Vec3d L = s_lights[i].isPoint ? (s_lights[i].positionOrdir - point).norm()
                                      : s_lights[i].positionOrdir.norm() * -1.0;
        ...
        float maxDist = 1e30f;
        if (s_lights[i].isPoint) {
            maxDist = (s_lights[i].positionOrdir - point).length();
        }

        float shadowFactor = computeShadowFactor_shared(point, L, maxDist, s_spheres, numSpheres,
                                                         bvhNodes, numBVHNodes, bvhTriIndices, 
                                                         triangles, s_nodeCache, cacheSize);

        totalDiffuse = totalDiffuse + diffuse * s_lights[i].intensity * shadowFactor;
        totalSpecular = totalSpecular + specular * s_lights[i].intensity * shadowFactor;
    }
...
// ---------------------------
// OPTIMIZED TILE-BASED KERNEL WITH SHARED MEMORY
// ---------------------------
#define TILE_SIZE 8
#define MAX_SHARED_SPHERES 32
#define MAX_SHARED_LIGHTS 16
#define MAX_SHARED_BVH_NODES 64

__global__ void raytrace_kernel_tiled_shmem(
    ...
    int tilesX)
{
    ...
    __shared__ DeviceSphere s_spheres[MAX_SHARED_SPHERES];
    __shared__ DeviceLight s_lights[MAX_SHARED_LIGHTS];
    __shared__ DeviceBVHNode s_bvhCache[MAX_SHARED_BVH_NODES];
    
    // Cooperative loading - each thread loads some data
    int tid = threadIdx.x;
    int numThreads = TILE_SIZE * TILE_SIZE;
    
    // Load spheres into shared memory
    int actualSpheres = min(numSpheres, MAX_SHARED_SPHERES);
    for (int i = tid; i < actualSpheres; i += numThreads) {
        s_spheres[i] = spheres[i];
    }
    
    // Load lights into shared memory
    int actualLights = min(numLights, MAX_SHARED_LIGHTS);
    for (int i = tid; i < actualLights; i += numThreads) {
        s_lights[i] = lights[i];
    }
    
    // Load BVH root nodes into shared memory (most frequently accessed)
    int actualBVHCache = min(numBVHNodes, MAX_SHARED_BVH_NODES);
    for (int i = tid; i < actualBVHCache; i += numThreads) {
        s_bvhCache[i] = bvhNodes[i];
    }
    ...
    int x = tileX * TILE_SIZE + localX;
    int y = tileY * TILE_SIZE + localY;
    ...
    // Sphere intersection - using shared memory
    float tClosestSphere = 1e30f;
    int sphereHitIndex = -1;
    Vec2d sphereTexCoord;
    Vec3d sphereNormal;

    for (int i = 0; i < actualSpheres; ++i) {
        float t;
        Vec2d tc;
        if (intersectRaySphere_device(ray, s_spheres[i], t, tc) && t > 0.001f && t < tClosestSphere) {
            tClosestSphere = t;
            sphereHitIndex = i;
            sphereTexCoord = tc;
            Vec3d hitPt(ray.ox + ray.dx * t, ray.oy + ray.dy * t, ray.oz + ray.dz * t);
            sphereNormal = (hitPt - s_spheres[i].center).norm();
        }
    }
    
    // Handle overflow spheres (if any)
    for (int i = MAX_SHARED_SPHERES; i < numSpheres; ++i) {
        float t;
        Vec2d tc;
    ...
    // Triangle intersection with cached BVH nodes
    float tClosestTri = 1e30f;
    int triHitIndex = -1;
    Vec3d triNormal;
    Vec2d triTexCoord;
    bool triHit = intersectBVH_shared(ray, bvhNodes, numBVHNodes, bvhTriIndices, triangles,
                                      s_bvhCache, actualBVHCache,
    ...
    if (hitType >= 0) {
        ...
        if (hitType == 0) {
            material = (hitIndex < actualSpheres) ? s_spheres[hitIndex].material : spheres[hitIndex].material;
            hasTexture = (hitIndex < actualSpheres) ? s_spheres[hitIndex].hasTexture : spheres[hitIndex].hasTexture;
        ...
        Color localColor = shade_shared(hitPoint, rayDir, hitNormal, hitTexCoord, material,
                                       s_lights, actualLights, s_spheres, actualSpheres,
                                       bvhNodes, numBVHNodes, bvhTriIndices, triangles,
                                       s_bvhCache, actualBVHCache,
                                       tex, hasTexture, bkgcolor);

        Color reflectedColor(0, 0, 0);
        if (material.ks > 0.0f && useSSR && imageBuffer && depthBuffer) {
            Vec3d reflDir = (rayDir - hitNormal * 2.0 * (rayDir.dot(hitNormal))).norm();
            reflectedColor = getSSRColor(reflDir, hitPoint, imageBuffer, depthBuffer,
                                        width, height, eyePos, viewDir, upDir, vfov, bkgcolor);
        }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Rendering " << width << "x" << height << " image with tiled BVH + SHARED MEMORY..." << std::endl;
    ...
    size_t shmem_spheres = MAX_SHARED_SPHERES * sizeof(DeviceSphere);
    size_t shmem_lights = MAX_SHARED_LIGHTS * sizeof(DeviceLight);
    size_t shmem_bvh = MAX_SHARED_BVH_NODES * sizeof(DeviceBVHNode);
    size_t total_shmem = shmem_spheres + shmem_lights + shmem_bvh;
    std::cout << "Shared memory usage per block: " << total_shmem << " bytes" << std::endl;
    std::cout << "  - Spheres: " << shmem_spheres << " bytes (" << MAX_SHARED_SPHERES << " max)" << std::endl;
    std::cout << "  - Lights: " << shmem_lights << " bytes (" << MAX_SHARED_LIGHTS << " max)" << std::endl;
    std::cout << "  - BVH cache: " << shmem_bvh << " bytes (" << MAX_SHARED_BVH_NODES << " nodes)" << std::endl;
    ...
    // Tile-based launch configuration
    int tilesX = (width + TILE_SIZE - 1) / TILE_SIZE;
    int tilesY = (height + TILE_SIZE - 1) / TILE_SIZE;
    int totalTiles = tilesX * tilesY;
    
    dim3 blockSize(TILE_SIZE * TILE_SIZE);
    dim3 gridSize(totalTiles);

    std::cout << "Launch config: " << totalTiles << " tiles (" << tilesX << "x" << tilesY 
              << "), " << blockSize.x << " threads/block" << std::endl;
    ...
    // Pass 1: Base image with depth
    raytrace_kernel_tiled_shmem<<<gridSize, blockSize>>>(
    ...
    std::cout << "GPU Rendering (tiled + shared memory, 2 passes) took " << (milliseconds / 1000.0f) << " seconds" << std::endl;

    CUDA_CHECK(cudaEventDestroy(start));
    CUDA_CHECK(cudaEventDestroy(stop));
    CUDA_CHECK(cudaGetLastError());

    CUDA_CHECK(cudaMemcpy(h_output, d_output, width * height * sizeof(Color), cudaMemcpyDeviceToHost));
    ...
    if (d_depthBuffer) CUDA_CHECK(cudaFree(d_depthBuffer));
