// raycast_ssr_cuda_bvh_tiled.cu
// Tile-based version for improved warp coherence
// 
// Key insight: Process rays in screen-space tiles (8x8).
// Rays from adjacent pixels have similar directions on smooth surfaces,
// so they traverse similar BVH paths → less divergence.
...
struct CompactRay {
    ...
    float invDx, invDy, invDz;  // 1/direction
};
...
// ---------------------------
// TILE-BASED KERNEL
// Each block processes an 8x8 tile of pixels
// Threads within a tile have coherent ray directions
// ---------------------------
#define TILE_SIZE 8

__global__ void raytrace_kernel_tiled(
    ...
    Color bkgcolor,
    bool useSSR,
    int tilesX)  // Number of tiles in X direction
{
    ...
    // BVH triangle intersection with warp-synchronized traversal
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Rendering " << width << "x" << height << " image with tiled BVH..." << std::endl;
    ...
    dim3 blockSize(TILE_SIZE * TILE_SIZE);  // 64 threads per block (8x8 tile)
    ...
    // Pass 1: Base image
    std::cout << "Pass 1: Base image..." << std::endl;
    raytrace_kernel_tiled<<<gridSize, blockSize>>>(
    ...
    // Pass 2: With SSR
    std::cout << "Pass 2: With SSR..." << std::endl;
    raytrace_kernel_tiled<<<gridSize, blockSize>>>(
    ...
    std::cout << "GPU Rendering (tiled, 2 passes) took " << (milliseconds / 1000.0f) << " seconds" << std::endl;
